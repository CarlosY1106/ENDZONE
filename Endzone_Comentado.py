...
# ---- Inicialización de Pygame y módulo de sonido ----
pygame.init()
pygame.mixer.init()
...
# ---- Función para cargar imágenes escaladas ----
...
menu_brasas = [{'x': random.randint(0, WIDTH), 'y': random.randint(0, HEIGHT),
                'speed': random.uniform(0.2, 0.6)} for _ in range(60)]
nubes = [{'x': random.randint(0, WIDTH), 'y': random.randint(0, HEIGHT),
          'radius': random.randint(50, 90), 'speed': random.uniform(0.05, 0.2),
          'alpha': random.randint(20, 40)} for _ in range(6)]

# ---- Función para dibujar un botón interactivo ----
def draw_button(text, x, y, w, h, base_color, hover_color, action=None):
    ...
    # Dibujar borde y relleno del botón
    pygame.draw.rect(screen, WHITE, rect, border_radius=10)
    inner_rect = rect.inflate(-4, -4)
    pygame.draw.rect(screen, hover_color if is_hovered else base_color,
                     inner_rect, border_radius=8)
    ...
    # Si el botón se presiona, ejecutar acción
...
def draw_health_bar(x, y, width, height, current_health, max_health,
                    border_color, fill_color, back_color, label="", label_pos="above"):
    # Texto de etiqueta (arriba o abajo)
...
def draw_nubes(nubes):
    for nube in nubes:
        ...
        pygame.draw.ellipse(cloud_surface, (200, 200, 200, nube['alpha']),
                            (0, 0, nube['radius']*2, nube['radius']))
        screen.blit(cloud_surface, (nube['x'], nube['y']))

# ---- Pantallas de información (historia, instrucciones, derrota, victoria) ----
def mostrar_pantalla_info(titulo, descripcion, volver_a_menu=False):
    ...
    while esperando:
        ...
        for i, linea in enumerate(descripcion.split("\n")):
            ...
            screen.blit(texto_surf, (WIDTH // 2 - texto_surf.get_width() // 2,
                                     HEIGHT // 2 - 60 + i * 35))
...
def game_loop():
    ...
    # ---- Partículas de brasas del nivel ----
    particles = [{'x': random.randint(0, WIDTH), 'y': random.randint(0, HEIGHT),
                  'radius': random.randint(1, 3), 'speed': random.uniform(0.2, 0.6)}
                 for _ in range(120)]

    # ---- Atributos del jugador ----
    player_pos = [WIDTH // 2, HEIGHT // 2]   # Posición inicial en el centro
    player_radius = 25                       # Tamaño del jugador
    base_speed = 4                           # Velocidad base
    player_speed = base_speed
    player_health = 100                      # Salud inicial
    player_max_health = 100
    player_xp = 0                            # Experiencia inicial
    player_level = 1                         # Nivel del jugador
    xp_to_next = 50                          # XP necesaria para subir nivel

    # ---- Atributos del juego ----
    nivel_actual = 1                         # Nivel del juego
    max_nivel = 3                            # Último nivel disponible
    ...
    projectiles = []
    projectile_speed = 7
    shoot_delay = 600  # Milisegundos entre disparos
    last_shot_time = pygame.time.get_ticks()

    # ---- Enemigos ----
    enemies = []
    enemy_spawn_delay = 1100   # Cada cuánto aparece un enemigo
    ...
    while running:
        ...
        # ---- Dibujar manchas de sangre con desvanecimiento ----
        ...
        # ---- Disparo automático hacia el mouse ----
        ...
        # ---- Colisiones: enemigo con jugador y proyectiles ----
        for enemy in enemies[:]:
                ...
                if math.hypot(enemy['x'] - p['x'], enemy['y'] - p['y']) < 30:
                    if enemy in enemies:
                        ...
                        if player_xp >= xp_to_next:
                            ...
                            elif player_level == 4 and nivel_actual == 3:
                                jefe_activo = True
                                jefe_pos = [random.randint(100, WIDTH - 100), random.randint(100, HEIGHT - 100)]
                                jefe_vida = jefe_max_vida = 15
                                jefe_speed = 3
                            ...
                            # Mejorar habilidad al subir de nivel
        ...
        if jefe_activo:
            ...
            if jefe_vida <= 0:
                ...
                # Si hay más niveles, mostrar historia
                ...
                # Mejoras según el nivel alcanzado
                if nivel_actual == 2:
                    habilidad_actual = 2
                    player_speed = base_speed
                elif nivel_actual == 3:
        ...
        # ---- Habilidad nivel 2 (ralentizar enemigos) ----
        if habilidad_actual == 2 and not ralentizar_enemigos and player_level >= 2:
            if keys[pygame.K_SPACE]:
                ...
                ralentizador_fin = now + 5000  # 5 segundos

        # ---- Habilidad nivel 3 (mayor velocidad) ----
        if habilidad_actual == 3:
            player_speed = base_speed * 1.8
        elif habilidad_actual in (1, 2):
            player_speed = base_speed

        # ---- Dibujar jugador ----
        screen.blit(personaje_img, (player_pos[0] - personaje_img.get_width() // 2,
                                    player_pos[1] - personaje_img.get_height() // 2))

        # ---- Dibujar enemigos ----
        for enemy in enemies:
            screen.blit(enemy['img'], (enemy['x'] - enemy['img'].get_width() // 2,
                                       enemy['y'] - enemy['img'].get_height() // 2))

        # ---- Dibujar proyectiles ----
        for p in projectiles:
            screen.blit(proyectil_img, (p['x'] - proyectil_img.get_width() // 2,
                                        p['y'] - proyectil_img.get_height() // 2))

        # ---- Dibujar jefe ----
        if jefe_activo:
            jefe_img = jefe_imgs.get(nivel_actual)
            if jefe_img:
                screen.blit(jefe_img, (int(jefe_pos[0] - jefe_img.get_width() // 2),
                                       int(jefe_pos[1] - jefe_img.get_height() // 2)))

            # Dibujar proyectiles del jefe
            for j in jefe_proyectiles:
                screen.blit(proyectil_jefe_img, (int(j['x'] - proyectil_jefe_img.get_width() // 2),
                                                 int(j['y'] - proyectil_jefe_img.get_height() // 2)))

            # Barra de vida del jefe
            jefe_labels = {1: "Barra de salud del Portador",
                           2: "Barra de salud del Acechador",
                           3: "Barra de salud del Susurrador"}
            draw_health_bar(WIDTH - 260, 50, 250, 25, jefe_vida, jefe_max_vida,
                            WHITE, RED, DARK_RED,
                            jefe_labels.get(nivel_actual, f"Jefe Fase {nivel_actual}"), "below")
        ...
        draw_health_bar(10, HEIGHT - 35, 200, 25, player_health, player_max_health,
                        WHITE, GREEN, DARK_RED, "Barra de salud de Rick", "above")

        # ---- Pantalla roja cuando hay daño ----
        if damage_flash > 0:
...
# ---- Bucle principal (menú, instrucciones, juego) ----
while True:
    clock.tick(60)
    if show_menu:
        screen.fill(BLACK)
        ...
        mostrar_pantalla_info(
            ...
            "Presiona ESPACIO para usar la habilidad del relentizador\n"
